...
require_locked_value \
    ...
    "$(sha256sum "$ROOT/scripts/native/mbedtls-config-srt.h" | cut -d' ' -f1)" \
...
# libsrt's crypto backend. The config at scripts/native/mbedtls-config-srt.h is the
...
MBEDTLS_CONFIG="$ROOT/scripts/native/mbedtls-config-srt.h"
MBEDTLS_FINGERPRINT="$(
    {
...
for helper in server client; do
    source_file="$ROOT/test/native/srt-bond-$helper.c"
