...
result=$(RESTREAM_MB_PER_TEST_THREAD=999999 run_nested 2>/dev/null || true)
...
# 2. Nested derivation produces a positive thread count.
echo "[nested-derivation]"
result=$(run_nested 2>/dev/null || true)
if [[ "$result" == "RUST_TEST_THREADS="* ]] && [[ -n "${result#RUST_TEST_THREADS=}" ]]; then
    pass "nested derivation works: $result"
else
    fail "nested derivation" "got: $result"
fi

# 3. Explicit RUST_TEST_THREADS override is preserved (skips memory/CPU
#    derivation entirely).
echo "[explicit-override]"
result=$(RUST_TEST_THREADS=2 run_nested 2>/dev/null || true)
...
exit_code=0
RUST_TEST_THREADS=0 RESTREAM_REPO_ROOT="$ROOT" RESTREAM_BUILD_LOCK_HELD=1 BUILD_JOBS=2 \
    scripts/build/resource-limit.sh --shared --timeout 5 \
