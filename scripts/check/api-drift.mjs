...
const directApiFetchAllowlist = new Set(["web/pages/login.html"]);
const routeLiteralAllowlist = new Set(["web/ts/core/api.ts", "web/pages/login.html"]);
const apiRequestAllowlist = new Set(["web/ts/core/api.ts"]);
  ...
  for (const file of files) {
    ...
    if (directFetchMatch && !directApiFetchAllowlist.has(file)) {
      pushViolation(
        ...
        "raw fetch('/api...') is only allowed in web/pages/login.html; use web/ts/core/api.ts",
      );
    }
    ...
    if (apiRequestMatch && !apiRequestAllowlist.has(file)) {
      pushViolation(
        ...
        "apiRequest() should only be called inside web/ts/core/api.ts",
      );
    }
    ...
    if (
      ...
      pushViolation(
        ...
        "route literals should live in web/ts/core/api.ts or web/pages/login.html",
      );
    }
  }
