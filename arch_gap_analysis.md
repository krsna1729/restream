...
| Phase H — Whole-codebase service and adapter split | ⚠️ Partial+ | Route modules, application services, runtime read-model service, graph planning, repository ports, and media FFmpeg/backend modules are split enough for Phases 1-16. Recording catalog concerns now live under `media/recording/catalog.rs` with the existing `media::recording::*` API preserved for callers, reducing media-library coupling to the recording runtime file. The larger idealized namespace split in `impl.md` (`RuntimeGraph`, registry modules, `media/protocols/*`, `media/hls/*`, recording runtime/writer modules) is still not fully realized. |
...
| Phase H Whole-codebase split | B+ | Services/routes/planner/runtime read models are split for Phase 1-16, and recording catalog helpers now live in `media/recording/catalog.rs`; broader `RuntimeGraph`, registry, protocol, HLS, and recording runtime/writer namespace splits remain partial. |
