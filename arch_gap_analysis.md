...
| Application services depend on repository traits | ⚠️ Partial | `PipelineService` and `HealthService` depend on `PipelineStore`, `OutputService` depends on `OutputStore`, and `IngestService` depends on `IngestLookup`/`IngestWriter`; `SettingsService`, `LogService`, and `AuthService` still hold `SqlitePool` and call `db::*`. |
...
3. **API/service/repository boundary remains mixed**
   - Route modules exist, but `api/agent.rs` and several services still call
     `db::*` directly. `PipelineService`, `OutputService`, `IngestService`, and
     `HealthService` are now port-trait backed.
...
| Ph 5 Repositories | B | Repo modules exist, and pipeline/output/ingest/health services are port-trait backed; several services still hold `SqlitePool` directly. |
