...
|---|---|---|
| Services exist | ✅ Present | `src/application/services/*` includes pipeline, output, ingest, file ingest, media library, settings, health, auth, logs, and agent context catalog assembly; `FileIngestService` now owns file-ingest FFmpeg argument/process setup. |
...
| Handlers do not call low-level media constructors | ⚠️ Mostly | `api/hls.rs` delegates to `application::hls_preview`, and file-ingest FFmpeg argument/process setup moved into `FileIngestService`; file-ingest start still performs runtime registration/spawn orchestration in `api/ingests.rs`, and other API/runtime views still take `MediaEngine` directly for read models. |
...
3. **API/service/repository boundary remains mixed**
   - Route modules exist, agent context catalog reads now use `AgentService`,
     and file-ingest FFmpeg argument/process setup now lives in
