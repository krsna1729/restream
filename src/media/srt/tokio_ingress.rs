...
impl SrtServer {
    ...
    pub async fn run(self: Arc<Self>, port: u16) {
        ...
        // Tokio readable + a 5ms maintenance tick. Do not park this
        // Tokio-owned UdpSocket in HighResWaiter.wait() / block_in_place:
        // #153 did that and quiet BBB N=30 sink/srt-only died with mass
        // egress leaf teardown (ACK path starved). HighResWaiter is a
        // dedicated-worker primitive, not a Tokio task waiter.
