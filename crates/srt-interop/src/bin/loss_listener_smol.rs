//! Sustained-throughput SRT listener over the pure-Rust Core, on smol --
//! the smol entry in the docs/srt-pure-rust-plan.md Phase 4
//! driver-framework bake-off. See loss_caller_smol.rs and
//! mio_driver.rs's doc comments for shared background.
//!
//! Usage: srt-interop-loss-listener-smol <port> <duration_secs> <latency_ms>
    ...
    smol::block_on(async {
        ...
        loop {
            if !connected && Instant::now() >= connect_deadline {
                eprintln!(
                    "[loss-listener-smol] connect timed out, state={:?}",
                    conn.state()
                );
                break;
            }
            ...
            // Drain any further packets already queued, non-blocking -- a
            // stray/malformed first datagram must not crash the process,
            // feed_recv_buf errors are always ignored here exactly like
            // every later packet.
            while let Some(res) = try_recv_from(&socket, &mut buf) {
                match res {
                    Ok((n, addr)) => {
                        if peer.is_none() {
                            if let Err(e) = socket.get_ref().connect(addr) {
            ...
            while let Some(ev) = conn.poll_event() {
                match ev {
                    ...
                    ConnectionEvent::Disconnected { reason } => {
                        eprintln!("[loss-listener-smol] disconnected: {reason}");
                        stream_deadline = Some(Instant::now());
                    }
                    ConnectionEvent::Error(msg) => {
                        eprintln!("[loss-listener-smol] error: {msg}");
                    }
                    _ => {}
                }
            }
        ...
        match stats {
            Some(s) => println!(
                "STATS role=listener backend=smol pkt_recv={total_received} pkt_recv_total={} \
            ...
            None => println!(
                "STATS role=listener backend=smol pkt_recv={total_received} pkt_recv_total=0 \
