//! Sustained-throughput SRT caller over the pure-Rust Core, on compio --
...
async fn main() {
    ...
    loop {
        if !connected && Instant::now() >= connect_deadline {
            eprintln!(
                "[loss-caller-compio] connect timed out, state={:?}",
                conn.state()
            );
            break;
        }
        ...
        while let Some(ev) = conn.poll_event() {
            match ev {
                ...
                ConnectionEvent::Disconnected { reason } => {
                    eprintln!("[loss-caller-compio] disconnected: {reason}");
                    stream_deadline = Some(Instant::now());
                }
                ConnectionEvent::Error(msg) => {
                    eprintln!("[loss-caller-compio] error: {msg}");
                }
                _ => {}
            }
        }
    ...
    match stats {
        Some(s) => println!(
            "STATS role=caller backend=compio pkt_sent={total_sent} pkt_sent_total={} \
        ...
        None => println!(
            "STATS role=caller backend=compio pkt_sent={total_sent} pkt_sent_total=0 \
