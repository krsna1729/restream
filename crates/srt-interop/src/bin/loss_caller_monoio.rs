//! Sustained-throughput SRT caller over the pure-Rust Core, on monoio --
//! the monoio entry in the docs/srt-pure-rust-plan.md Phase 4
//! driver-framework bake-off. monoio is completion-based (io_uring):
//! operations take *owned* buffers and hand them back on completion,
//! unlike mio/tokio/smol's readiness model (borrow a buffer, get told when
//! it's safe to read/write it). That ownership-transfer shape is the real
//! architectural difference under test here, not just "another reactor" --
//! see mio_driver.rs's doc comment for shared background and Cargo.toml's
//! doc comment for why each backend is a separate binary.
//!
//! Usage: srt-interop-loss-caller-monoio <host> <port> <duration_secs> <latency_ms> [bitrate_bps]
//!
//! No drain-all-pending-packets loop here unlike the readiness-based
//! backends: completion I/O has no "is it ready yet" check to loop on,
//! only "await the next completion." Each outer loop iteration awaits at
//! most one recv (raced against the pacing deadline), which is fine
//! because the loop itself cycles fast under the same
//! sleep-the-bulk-spin-the-tail pacing split used elsewhere.
...
async fn main() {
    ...
    loop {
        if !connected && Instant::now() >= connect_deadline {
            eprintln!(
                "[loss-caller-monoio] connect timed out, state={:?}",
                conn.state()
            );
            break;
        }
        ...
        if block_for > Duration::ZERO {
            // A fresh buffer per attempt, not a reused/reassigned one:
            // io_uring ops can't be safely cancelled mid-flight, so if the
            // timeout branch wins the race here, this buffer's ownership
            // is simply abandoned along with the (still-outstanding, from
            // the kernel's perspective) recv op -- monoio::time::timeout
            // documents this as the expected pattern rather than something
            // this code works around.
            if let Ok((res, buf)) =
                monoio::time::timeout(block_for, socket.recv(vec![0u8; 2048])).await
                && let Ok(n) = res
            {
                let t = now(start);
        ...
        while let Some(ev) = conn.poll_event() {
            match ev {
                ...
                ConnectionEvent::Disconnected { reason } => {
                    eprintln!("[loss-caller-monoio] disconnected: {reason}");
                    stream_deadline = Some(Instant::now());
                }
                ConnectionEvent::Error(msg) => {
                    eprintln!("[loss-caller-monoio] error: {msg}");
                }
                _ => {}
            }
        }
    ...
    match stats {
        Some(s) => println!(
            "STATS role=caller backend=monoio pkt_sent={total_sent} pkt_sent_total={} \
        ...
        None => println!(
            "STATS role=caller backend=monoio pkt_sent={total_sent} pkt_sent_total=0 \
