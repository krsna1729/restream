//! Sustained-throughput SRT caller over the pure-Rust Core, on glommio --
//! the glommio entry in the docs/srt-pure-rust-plan.md Phase 4
//! driver-framework bake-off. Glommio is thread-per-core io_uring like
//! monoio, but (unlike monoio) exposes a borrowed-buffer recv/send API,
//! not owned-buffer completion semantics -- its own internal buffer
//! management over io_uring, closer in shape to mio/tokio/smol from this
//! code's point of view. Also brings built-in per-task-queue CPU/latency
//! quotas (not exercised by this single-connection benchmark, which has
//! only one task queue). See mio_driver.rs's doc comment for shared
//! background and Cargo.toml's doc comment for why each backend is a
//! separate binary.
//!
//! Usage: srt-interop-loss-caller-glommio <host> <port> <duration_secs> <latency_ms> [bitrate_bps]
//!
...
fn main() {
    eprintln!("srt-interop-loss-caller-glommio: Linux-only (io_uring)");
    std::process::exit(2);
}
    ...
    pub fn run() {
        ...
        let exit_connected = LocalExecutorBuilder::default()
            .spawn(move || async move {
                ...
                let now =
                    |start: Instant| Timestamp::from_micros(start.elapsed().as_micros() as u64);
                ...
                loop {
                    if !connected && Instant::now() >= connect_deadline {
                        eprintln!(
                            "[loss-caller-glommio] connect timed out, state={:?}",
                            conn.state()
                        );
                        break;
                    }
                    ...
                    while let Some(ev) = conn.poll_event() {
                        match ev {
                            ...
                            ConnectionEvent::Disconnected { reason } => {
                                eprintln!("[loss-caller-glommio] disconnected: {reason}");
                                stream_deadline = Some(Instant::now());
                            }
                            ConnectionEvent::Error(msg) => {
                                eprintln!("[loss-caller-glommio] error: {msg}");
                            }
                            _ => {}
                        }
                    }
                ...
                match stats {
                    Some(s) => println!(
                        "STATS role=caller backend=glommio pkt_sent={total_sent} pkt_sent_total={} \
                    ...
                    None => println!(
                        "STATS role=caller backend=glommio pkt_sent={total_sent} pkt_sent_total=0 \
