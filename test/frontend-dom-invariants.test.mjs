...
test("static HTML keeps core DOM accessibility and layout invariants", async () => {
  ...
  assert.deepEqual(duplicateIds, [], "HTML IDs must be unique across shipped pages");
  assert.match(indexHtml, /<meta name="viewport" content="width=device-width, initial-scale=1" \/>/);
  assert.match(loginHtml, /<meta name="viewport" content="width=device-width, initial-scale=1" \/>/);
  ...
  assert.match(indexHtml, /min-w-0 overflow-y-auto rounded-lg border p-4 xl:min-w-\[24rem\]/);
...
test("dashboard grid sizing lives in responsive CSS instead of inline scripts", async () => {
  const [inputCss, renderTs] = await Promise.all([
    readPublicFile("input.css"),
    readFile(new URL("../public/ts/features/render.ts", import.meta.url), "utf8"),
  ]);

  assert.match(inputCss, /#dashboard-grid\s*{\s*grid-template-columns: minmax\(0, 1fr\);/s);
  assert.match(inputCss, /#dashboard-grid\.has-selected-pipeline/s);
  assert.match(renderTs, /classList\.toggle\("has-selected-pipeline"/);
  assert.doesNotMatch(renderTs, /minmax\(24rem,\s*34rem\).*minmax\(24rem,\s*1fr\)/s);
});
