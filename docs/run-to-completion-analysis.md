...
```mermaid
flowchart LR
    Tokio["ASYNC RUNTIME<br/>(Tokio)<br/>Socket recv<br/>Demuxing<br/>Muxing<br/>Filtering"]

    Boundary["MemoryQueue<br/>Condvar"]

    Threads["OS THREADS<br/>srt_send()<br/>FFmpeg<br/>H.265→H.264<br/>fwrite()"]

    Tokio -->|Decoupling| Boundary
    Boundary -->|Isolate| Threads

    Why["Why Essential<br/>Tokio blocked ⟹<br/>all tasks stalled<br/>FFmpeg: 0.5-2s<br/>libsrt: network<br/>Disk: 100+ ms"]
```
...
```mermaid
flowchart LR
    R1["1. HLS<br/>segmenting<br/> High<br/>1 Decoup<br/>Mutex"]

    R2["2. RTMP→RTMP<br/>passthrough<br/> Medium<br/>1 ring<br/>Multi-C"]

    R3["3. Audio<br/>routing<br/> Medium<br/>1 ring<br/>Multi-C"]

    R4["4. SRT<br/>demux<br/> Medium<br/>1 ring<br/>Multi-C"]

    R5["5. SRT→SRT<br/>pass<br/> Low<br/>3 Decoup<br/>srt_send"]

    R6["6. SRT→RTMP<br/>codec<br/> Low<br/>2 Decoup<br/>AVCC"]

    R7["7. Rec<br/>disk<br/> Low<br/>2 Decoup<br/>Disk I/O"]

    R8["8. Multi<br/>audio<br/> Low<br/>2-3 rings<br/>srt_send"]

    R9["9. RTMP<br/>720p<br/> Very Low<br/>4 Decoup<br/>FFmpeg"]

    R10["10. SRT<br/>H.265<br/> Very Low<br/>6+ Decoup<br/>FFmpeg"]
...
```mermaid
flowchart LR
    T0["0 μs<br/>Ingest<br/>socket read<br/>+ parse<br/>~0.5-3 μs<br/>RTMP/SRT"]

    T1["~3 μs<br/>Source<br/>RingBuffer<br/>Arc+publish<br/>~0.15 μs"]

    T2["~3.2 μs<br/>Reader<br/>acquire<br/>per egress<br/>~0.03 μs"]

    T3["~3.25 μs<br/>Egress<br/>processing<br/>RTMP: 0.1 μs<br/>SRT: 0.6 μs<br/>Codec: 0.7-2 μs<br/>H.265: 5-50 ms<br/>HLS: 0.6 μs"]

    T4["~4 μs<br/>TsChunkRing<br/>SRT path<br/>only<br/>~0.1 μs"]

    T5["~4.1 μs<br/>MemoryQueue<br/>write<br/>SRT path<br/>Mutex: 0.05 μs<br/>Push: 0.01 μs<br/>Notify: 0.1 μs"]

    T6["~4.3 μs<br/>OS thread<br/>wakeup<br/>context switch<br/>~0.5-5 ms"]

    T7["~4.3+ms<br/>libsrt_send<br/>blocking<br/>Network I/O<br/>~1-100+ ms<br/>UDP kernel buffer"]

    T8["~100+ms<br/>Packet exits<br/>Network latency"]

    T0 --> T1 --> T2 --> T3
    T3 --> T4
    T4 --> T5 --> T6 --> T7 --> T8

    Transcoding["Transcoding<br/>Add ~0.5-2000 ms<br/>FFmpeg codec work"]
...
flowchart TD
    ...
    FFmpeg["FFmpeg Sharding<br/>(pyramid parallelization)<br/>Effort: HIGH<br/>Gain: Encode 2-3x<br/>Impact: HIGH"]

    Ring["Adaptive Ring Sizing<br/>(single-output opt)<br/>Effort: MEDIUM<br/>Gain: RSS -20-30%<br/>Impact: MEDIUM"]

    IO["Async I/O (io-uring)<br/>for recording<br/>Effort: MEDIUM-HIGH<br/>Gain: Latency -5-10%<br/>Impact: MEDIUM"]

    HLS["Lock-free HLS<br/>segment store<br/>Effort: LOW<br/>Gain: <1% latency<br/>Impact: VERY LOW"]

    Priority["Recommended Priority<br/>1. Measure FFmpeg bottleneck<br/>2. If encode-bound: sharding<br/>3. If SRT dominant: async send<br/>4. General latency: optimized "]
...
```mermaid
flowchart TD
    START["START<br/>Can path run<br/>end-to-end?"]
    ...
    Q2_YES["YES<br/>Need isolation<br/><br/>→ Source RingBuffer"]
    ...
    Q6_NO["RTMP passthrough<br/>Ring-only (unavoidable)"]
