...
| SRT egress local-port reuse | Enabled | `RESTREAM_SRT_EGRESS_REUSE_LOCAL_PORT` (`0`/`false` gives every SRT output isolated native UDP state; enabled creates one application-owned nonblocking UDP socket/poller per local address family plus an srt-rs logical caller table per egress-fabric shard. Unrelated to `RESTREAM_SRT_EGRESS_MUXER_MAX_OUTPUTS_PER_SHARD`, which shards the shared TS muxer stage.) |
...
| SRT receive budget | `64` datagrams | `RESTREAM_SRT_RECV_BUDGET_DATAGRAMS` (A/B knob; bounds each native ingress and egress receive drain. The measurement sink keeps `RecvBudget::new(8, 512)` unless this env is set.) |
