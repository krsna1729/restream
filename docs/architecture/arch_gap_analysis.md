...
`scripts/harness/rollouts/internal-video-presets.sh`. The external-capacity guard
is captured in `scripts/harness/rollouts/external-capacity.sh`: the capacity-ok
...
Correctness proof added or verified for this pass now includes **14
...
- Proptest/property coverage: ADTS frame counting, AVIO chunk round-trip,
  ...
  planner stage-identity invariants, audio-router selected-track invariants,
...
|---|---|---|
| Source inventory doc / CI | ✅ Present | `scripts/check/source-audit.sh` checks forbidden imports, no-growth file-size baselines, and env reads; it emits `target/source-audit.json` and now runs in the CI architecture-guardrails job. |
...
| Forbidden-import CI check | ✅ Present | `ARCHITECTURE_GUARDRAILS.md` documents the boundary rules, and CI runs `scripts/check/source-audit.sh` to reject `src/media` imports from API modules. |
...
| No env reads outside config/startup/test harness | ✅ Complete for phase scope | `scripts/check/source-audit.sh` reports no raw `std::env::var` usage outside configuration; the remaining production constructor call is `MediaEngine::new()` delegating to `AppConfig::from_env()`, and direct env reads outside config are limited to startup, MCP, tests, and harness/process utilities. |
...
| Ph 15 Large-file split | A | No audited Rust, TypeScript, or hand-written frontend JS/MJS test file exceeds 2,000 lines; `scripts/check/source-audit.sh` now enforces that cap, and the extracted modules are below the ideal cap through responsibility-based splits. |
...
| Phase I Harness governor | A | Exact named governor tests, stronger source-audit checks, fast-breadth root-cause artifacts, and bounded CI/release harness lanes now exist. |
