...
| `MSR_SRT_OUTPUT_TARGET` | unset | Test-only SRT output `host:port` override for a controlled receiver; when bonding is enabled, both legs use this same target. Unset preserves the normal per-peer `MTX_SRT` target |
