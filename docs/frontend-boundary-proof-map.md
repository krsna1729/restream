...
| SSE/log-stream reconnect | One connection per filter/scope, paused while the tab is hidden, resumed from the last event id on visibility, replaced (not duplicated) when scope changes, and terminal closure is reported rather than retried forever. | `frontend-log-stream.test.mjs`, `frontend-status-stream.test.mjs`, `frontend-history-stream.test.mjs`, `frontend-overview-activity-stream.test.mjs`. | None of these prove behavior under randomized event/visibility interleaving yet; a fake-timers ordering test for one stream is the natural next layer (see `docs/testing.md`'s property/interleaving-test guidance). |
