...
`url` supports `rtmp://`, `rtmps://`, and `srt://` output targets.
...
For SRT outputs, FFmpeg uses `-f mpegts` for egress container output. RTMP/RTMPS outputs continue using `-f flv` with RTMP flags.
