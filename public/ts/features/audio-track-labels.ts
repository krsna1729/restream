...
function isMeaningfulLanguage(lang: string | null | undefined): lang is string {
  const t = lang?.trim();
...
export function audioTrackIdentifier(
  track: AudioTrack | null | undefined,
  position: number,
): string {
  const parts: string[] = [];
  ...
  if (isMeaningfulLanguage(language)) {
    parts.push(String(language).toUpperCase());
  }
  return parts.join(" / ");
}
...
export function getAudioTrackLabel(
  ...
  const stored =
    readLabels()[pipelineId]?.[audioTrackKey(track, position)]?.trim();
  if (stored) return stored;
  if (track?.title?.trim()) return track.title.trim();
  const language = track?.language;
  if (isMeaningfulLanguage(language)) return language.trim().toUpperCase();
  if (Number.isFinite(track?.index as number))
