...
/// Caller-owned destination for one encoded SRT datagram.
///
/// The transport never retains the borrowed packet. Returning `false` keeps
...
struct VecPacketSink<'a> {
    out: &'a mut Vec<(std::net::SocketAddr, Vec<u8>)>,
}

impl PacketSink for VecPacketSink<'_> {
    fn push(&mut self, peer: std::net::SocketAddr, packet: Vec<u8>) -> Result<(), Vec<u8>> {
        self.out.push((peer, packet));
        Ok(())
    }
}
...
struct DrainSink<'a, S: PacketSink + ?Sized> {
...
impl CallerSession {
    ...
    fn drain_one<S: PacketSink + ?Sized>(
        &mut self,
        now: Timestamp,
...
impl CallerTable {
    ...
    /// Fairly drain bounded protocol work into caller-owned TX storage.
    ///
    /// `sink` is called while each packet is borrowed and must copy or submit
    /// it before returning. A rejected packet remains queued, preserving the
    /// SRT timer and pacing state for the next readiness visit.
    pub fn poll_outbound_into<S: DatagramSink>(
        &mut self,
        now: Timestamp,
        budget: OutputDrainBudget,
        sink: &mut S,
    ...
    fn drain_ready_bounded<S: PacketSink + ?Sized>(
        &mut self,
        now: Timestamp,
        budget: OutputDrainBudget,
        output: &mut S,
...
fn drain_one_caller_leg<S: PacketSink + ?Sized>(
...
fn drain_one_caller_leg_parts<S: PacketSink + ?Sized>(
    peer: std::net::SocketAddr,
    now: Timestamp,
    ...
    match output {
        ConnectionOutput::SendPacket(packet) => {
            ...
            match sink.sink.push(peer, packet) {
                Ok(()) => (DrainOne::Drained, false),
                Err(packet) => {
...
mod tests {
    ...
    #[test]
    fn caller_owned_sink_applies_backpressure_without_dropping_packet() {
