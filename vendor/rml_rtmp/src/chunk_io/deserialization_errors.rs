...
pub enum ChunkDeserializationError {
    ...
    /// A chunk header declared a message longer than the configured maximum
    /// (`ChunkDeserializer::set_max_message_length`). Raised when the length is
    /// decoded, before any payload is buffered. (restream vendor patch)
