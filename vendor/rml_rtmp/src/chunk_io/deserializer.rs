...
/// RTMP message lengths are 24-bit.
const PROTOCOL_MAX_MESSAGE_LENGTH: usize = 0x00FF_FFFF;
...
impl ChunkDeserializer {
    ...
    pub fn new() -> ChunkDeserializer {
        ChunkDeserializer {
            ...
            max_message_length: PROTOCOL_MAX_MESSAGE_LENGTH,
    ...
    /// Reject any message whose declared length exceeds `max_length` bytes, as
    /// soon as the length is decoded and before any of its payload is
    /// buffered. Defaults to the protocol maximum (24-bit length), which keeps
    /// upstream behavior. (restream vendor patch)
    pub fn set_max_message_length(&mut self, max_length: usize) {
    ...
    pub fn in_progress_bytes(&self) -> usize {
        self.buffer.len() + self.current_payload_data.len()
    }
    ...
    fn get_message_length(&mut self) -> Result<ParseStageResult, ChunkDeserializationError> {
        ...
        if length as usize > self.max_message_length {
    ...
    fn get_message_data(
        ...
        // restream vendor patch: the payload grows with the bytes that actually
        // arrived instead of reserving the whole declared message length on
        // its first chunk, which let one small chunk force a 16 MiB allocation.
...
mod tests {
    ...
    /// Type 0 chunk header on csid 3 declaring `declared_length`, followed by
    /// `sent` payload bytes. (restream vendor patch tests)
    fn declared_chunk(declared_length: u32, sent: usize) -> Vec<u8> {
        let mut bytes = vec![0x03, 0, 0, 0];
    ...
    #[test]
    fn oversized_declared_message_is_rejected_before_payload_is_buffered() {
        let mut deserializer = ChunkDeserializer::new();
        deserializer.set_max_message_length(1_000);
        let error = deserializer
            .get_next_message(&declared_chunk(1_000_000, 128))
            .expect_err("a declared length over the maximum must fail");
        assert!(matches!(
            error,
        ...
        assert_eq!(deserializer.current_payload_data.len(), 0);
    }

    #[test]
    fn a_large_declared_message_grows_with_received_bytes_not_its_declaration() {
        let mut deserializer = ChunkDeserializer::new();
        let result = deserializer
            .get_next_message(&declared_chunk(0x00FF_FFFF, 128))
            .unwrap();
        assert!(result.is_none(), "the message is incomplete");
        assert_eq!(deserializer.in_progress_bytes(), 128);
        assert!(
            deserializer.current_payload_data.capacity() < 64 * 1024,
            "declared 16 MiB must not be reserved up front, got {}",
            deserializer.current_payload_data.capacity()
        );
    }

    #[test]
    fn a_message_at_the_maximum_is_still_accepted() {
        let mut deserializer = ChunkDeserializer::new();
        deserializer.set_max_message_length(100);
        let message = deserializer
            .get_next_message(&declared_chunk(100, 100))
            .unwrap()
            .expect("a message exactly at the maximum completes");
